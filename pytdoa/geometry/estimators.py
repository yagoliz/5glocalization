...
def dop(positions: ndarray_f64, receivers: ndarray_f64) -> ndarray_f64:
    """
    ...
    assert (
        n == receivers.shape[1]
    ), "Incompatible number of dimensions for positions and receivers"

    # We generate a 1xnxm hypermatrix and extend it with receivers
    d_vec = np.transpose(
        positions.reshape(m, n, 1), axes=[2, 1, 0]
    ) - receivers.reshape(r, n, 1)
    ...
    Q = np.linalg.inv(
        np.matmul(np.transpose(H, axes=[2, 1, 0]), np.transpose(H, axes=[2, 0, 1]))
    )
...
def crlb(
    positions: ndarray_f64,
    receivers: ndarray_f64,
